...
const npm_youtube_transcript_1 = require("npm:youtube-transcript"); // Added for YouTube
const pdf_mjs_1 = require("https://esm.sh/pdf.mjs"); // PDF text extraction compatible with edge runtime
...
async function updateDocumentStatus(supabase, documentId, status, metadataUpdate = {}) {
    ...
    const newMetadata = Object.assign(Object.assign({}, ((currentDoc === null || currentDoc === void 0 ? void 0 : currentDoc.metadata) || {})), metadataUpdate);
...
async function extractTextFromPdf(filePath, supabase, bucketName) {
    ...
    try {
        ...
        if (downloadError || !pdfFileData) {
            throw new Error(`Failed to download PDF file ${filePath} from storage: ${downloadError === null || downloadError === void 0 ? void 0 : downloadError.message}`);
        }
        const pdfBuffer = await pdfFileData.arrayBuffer();
        const uint8 = new Uint8Array(pdfBuffer);
        const loadingTask = (0, pdf_mjs_1.getDocument)({ data: uint8 });
        const pdfDoc = await loadingTask.promise;
        let allText = '';
        for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum++) {
            const page = await pdfDoc.getPage(pageNum);
            const textContent = await page.getTextContent();
            const pageStrings = textContent.items.map((itm) => itm.str || '').join(' ');
            allText += pageStrings + '\n\n';
        }
...
async function extractTextFromUrl(url) {
    ...
    for (const strategy of strategies) {
        try {
            ...
            const response = await fetch(url, Object.assign(Object.assign({}, strategy.options), { signal: controller.signal }));
    ...
    const errorMessage = `Failed to extract text from URL ${url} after trying ${strategies.length} different approaches. Last error: ${(lastError === null || lastError === void 0 ? void 0 : lastError.message) || 'Unknown error'}`;
    console.error(errorMessage);
    // Provide helpful error message based on the type of error
    let userFriendlyMessage = `Unable to access the website at ${url}. `;
    if ((lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('timeout')) || (lastError === null || lastError === void 0 ? void 0 : lastError.name) === 'AbortError') {
        userFriendlyMessage += 'The website took too long to respond (timeout after 30 seconds).';
    }
    else if ((lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 403')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 401'))) {
        userFriendlyMessage += 'The website is blocking automated access (403/401 error).';
    }
    else if (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 404')) {
        userFriendlyMessage += 'The page was not found (404 error). Please check the URL.';
    }
    else if ((lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 500')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 502')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('HTTP 503'))) {
        userFriendlyMessage += 'The website is experiencing server issues. Please try again later.';
    }
    else if ((lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('SSL')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('TLS')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('certificate'))) {
        userFriendlyMessage += 'There was an SSL/security certificate issue with the website.';
    }
    else if ((lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('network')) || (lastError === null || lastError === void 0 ? void 0 : lastError.message.includes('connection'))) {
...
async function extractTranscriptFromYouTube(url) {
    ...
    try {
        ...
        let transcriptResponse = await npm_youtube_transcript_1.YoutubeTranscript.fetchTranscript(url);
        ...
        if (!transcriptResponse || transcriptResponse.length === 0) {
            ...
            try {
                transcriptResponse = await npm_youtube_transcript_1.YoutubeTranscript.fetchTranscript(url, { lang: 'en' });
            }
            catch (langErr) {
        ...
        if (!transcriptResponse || transcriptResponse.length === 0) {
            ...
            for (const lang of languagesToTry) {
                try {
                    ...
                    transcriptResponse = await npm_youtube_transcript_1.YoutubeTranscript.fetchTranscript(url, { lang });
                    if (transcriptResponse && transcriptResponse.length > 0) {
...
async function tryAlternativeTranscriptMethods(url) {
    try {
        ...
        for (const country of countryCodes) {
            try {
                ...
                const transcriptResponse = await npm_youtube_transcript_1.YoutubeTranscript.fetchTranscript(url, {
                    lang: 'en',
...
async function transcribeAudio(filePath, supabase, bucketName, openaiApiKey) {
    ...
    try {
        ...
        if (downloadError || !audioFileData) {
            throw new Error(`Failed to download audio file ${filePath} from storage: ${downloadError === null || downloadError === void 0 ? void 0 : downloadError.message}`);
        }
...
const getSectionIdentifier = (documentType, content, // Can be the full text or a segment relevant to the current chunk
charOffset, // Character offset of the chunk within the full text
chunkIndex, // Index of the current chunk
pageNumber, // For PDFs
timestamp // For YouTube
) => {
...
const generateCitationKey = (docId, sectionIdentifier, chunkIndex) => {
...
const chunkText = async (fullText, documentType, options = {}) => {
...
(0, server_ts_1.serve)(async (req) => {
    ...
    let documentId = null;
    let supabaseClient = null; // Use this for most operations
    try {
        const { documentId: reqDocumentId } = await req.json();
        ...
        supabaseClient = (0, supabase_js_2_1.createClient)(supabaseUrl, supabaseServiceRoleKey);
        ...
        const { data: document, error: fetchError } = await supabaseClient
            ...
            .single();
        if (fetchError)
            throw new Error(`DB fetch error for ${documentId}: ${fetchError.message}`);
        if (!document)
            throw new Error(`Document not found: ${documentId}`);
        let extractedText = "";
        const docMetadata = document.metadata || {};
        const fileType = document.file_type || '';
        const sourceUrl = docMetadata.originalUrl || '';
        ...
        if (sourceUrl && (sourceUrl.includes('youtube.com') || sourceUrl.includes('youtu.be'))) {
            ...
            const videoMetadata = {
                video_id: videoId,
                video_title: (videoInfo === null || videoInfo === void 0 ? void 0 : videoInfo.title) || 'Unknown',
                video_duration: (videoInfo === null || videoInfo === void 0 ? void 0 : videoInfo.duration) || null,
                channel_name: (videoInfo === null || videoInfo === void 0 ? void 0 : videoInfo.author_name) || null,
                processing_attempts: 1,
                strategies_tried: [],
                processing_start_time: new Date().toISOString()
            };
            await updateDocumentStatus(supabaseClient, documentId, 'processing', Object.assign(Object.assign({}, videoMetadata), { processing_stage: 'transcript_extraction' }));
            ...
            if (transcriptResult.error) {
                ...
                const errorMetadata = Object.assign(Object.assign({}, videoMetadata), { error_type: errorType, error_message: transcriptResult.message, processing_completed_at: new Date().toISOString(), user_guidance: transcriptResult.message, final_strategy_used: 'none', processing_outcome: 'error' });
                ...
                const successMetadata = Object.assign(Object.assign({}, videoMetadata), { final_strategy_used: 'transcript_fetch_multilang', processing_outcome: 'success', transcript_length: extractedText.length, processing_stage: 'chunking' });
            ...
            if (downloadError || !fileData)
                throw new Error(`Storage download failed for ${storagePath}: ${downloadError === null || downloadError === void 0 ? void 0 : downloadError.message}`);
        ...
        const processedChunks = await chunkText(extractedText, fileType, { chunkSize: 1500, overlap: 200 });
